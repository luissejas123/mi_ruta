> **Archivado 2026-09-13.** Su contenido quedó superado por la revisión completa de documentación (Documentos 1-10 en `docs/specs/`, ver README) y por `docs/DEUDA_TECNICA.md`/`docs/PLAN_SEGURIDAD_TARIFAS_GPS.md`, que ya reflejan el estado real y vigente de roles/jerarquía. Se archiva como historial, no como referencia activa. Estado del proyecto a esta fecha: **Sprint 3 avanzado y parcialmente completado**; varios módulos que este documento reservaba para "Sprint 4" (chofer/admin/tickeador/presidente) **ya se adelantaron** y están implementados — el roadmap real terminó corriendo Sprint 3 y 4 en paralelo, no en secuencia, algo típico de Scrum cuando el cliente pide cambios sobre la marcha. El índice compuesto de Firestore que menciona §11 ya está resuelto — es uno de los 7 que documenta `docs/PLAN_SEGURIDAD_TARIFAS_GPS.md`.
