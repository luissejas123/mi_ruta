> **Archivado 2026-09-13.** Superado por la revisión completa de documentación (Documentos 1-10 en `docs/specs/`, ver README) — el Documento 2 (Diseño Funcional) y el Documento 7 (Planeación del Desarrollo) ya capturan, con evidencia de código verificada, el mismo diagnóstico de reparto de responsabilidades por perfil que este documento proponía (Presidente con más carga real que la documentada, Administrador con menos). Estado del proyecto a esta fecha: Sprint 3 avanzado y parcialmente completado; Sprint 4 ya se está adelantando en paralelo. `RQ4-TIC-01` (qué implementación de Tickeador es la base) sigue abierto — ya trackeado en `docs/DEUDA_TECNICA.md` ítem 2. **`RQ4-PRE-05` (§7.3: cuál de las dos "vista presidente" casi idénticas de Figma, nodos `3412:9656` vs `3412:9732`, es la definitiva) queda sin resolver y no está trackeado en ningún otro documento activo** — si se retoma, hay que decidirlo primero.
