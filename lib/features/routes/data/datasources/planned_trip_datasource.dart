...
class PlannedTripDatasource {
  ...
  Future<List<PlannedTrip>> getAll(String userId) async {
    final snap = await _col(userId)
        ...
        .limit(50)
        .get();
    return snap.docs.map((d) => _fromDoc(d, userId)).toList();
  }
  ...
  PlannedTrip _fromDoc(DocumentSnapshot doc, String userId) {
    ...
    return PlannedTrip(
      ...
      originLatLng:
          LatLng(d['origin_lat'] as double, d['origin_lng'] as double),
      destinationName: d['destination_name'] as String,
      destinationLatLng: LatLng(
          d['destination_lat'] as double, d['destination_lng'] as double),
  ...
  PlannedTripLeg _legFromMap(Map<String, dynamic> m) {
    ...
    final type =
        typeStr == 'walking' ? LegType.walking : LegType.bus;
    return PlannedTripLeg(
      ...
      boardingPoint:
          LatLng(m['boarding_lat'] as double, m['boarding_lng'] as double),
      alightingPoint:
          LatLng(m['alighting_lat'] as double, m['alighting_lng'] as double),
